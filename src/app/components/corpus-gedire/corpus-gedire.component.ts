...
@Component({
  selector: 'app-corpus-gedire',
  templateUrl: './corpus-gedire.component.html',
  styleUrls: ['./corpus-gedire.component.css']
})
export class CorpusGedireComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.watchStorage();
    this.loadContent('corpus-gedire');
  ...
  saveContent(){
    ...
    let action = {'title': 'edit', 'datasheet': {'oldName': this.content.data.corpus[this.newContent.index].name, 'newName': this.newContent.name, 'type': 'corpus-gedire'}};
  ...
  deleteContent(index){
    ...
    let action = {'title': 'delete', 'datasheet': {'oldName': this.content.data.corpus[index].name, 'type': 'corpus-gedire'}};
