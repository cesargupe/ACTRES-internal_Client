...
@Component({
  selector: 'app-corpus-corpes',
  templateUrl: './corpus-corpes.component.html',
  styleUrls: ['./corpus-corpes.component.css']
})
export class CorpusCorpesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.watchStorage();
    this.loadContent('corpus-corpes');
  ...
  saveContent(){
    ...
    let action = {'title': 'edit', 'datasheet': {'oldName': this.content.data.corpus[this.newContent.index].name, 'newName': this.newContent.name, 'type': 'corpus-corpes'}};
  ...
  deleteContent(index){
    ...
    let action = {'title': 'delete', 'datasheet': {'oldName': this.content.data.corpus[index].name, 'type': 'corpus-corpes'}};
